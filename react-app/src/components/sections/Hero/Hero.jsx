...
export function Hero() {
    const nameText = "Shreyas Suhas Yadav";
    ...
    const titleLines = useMemo(() => {
        const lines = [
            { text: 'Building', word: 'Building' },
            { text: 'the Future', word: 'the Future' }
        ];
        ...
        return lines.map((line, lineIndex) => {
            const chars = line.text.split('').map((char, i) => {
                if (char === ' ') {
                    return { char: '\u00A0', index: i, charIndex: -1, globalIndex: globalCharIndex++ };
                }
                const result = { char, index: i, charIndex: globalCharIndex, globalIndex: globalCharIndex };
                globalCharIndex++;
                return result;
            });
